The aim of this script is to segment spots in up to 3 channels and determine intensity, volume, center of mass characteristics. This script also provides two-way partnership measurements based on voxel overlap and distance thresholding.
...
Additionally please install `DiAna 1.50`. The latest version of DiAna can be found [here](https://imagej.net/plugins/distance-analysis#installation). An archived copy of `DiAna 1.50` is also in [this repository](https://github.com/FenaOchs/Ochs_et_al.2023/tree/main/SIMinspector_Subobject_Fiji/DiAna_1.50).
...
Inspect the images and check whether the parameters seem correct. If not, try different thresholds and parameters, then re-run the macro with the newly selected values.
...
* Depending on the segmentation algorithm used you may want to erode the labels to optimise segmentation.
